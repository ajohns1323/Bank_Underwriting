...
first.second.test$PD <- scenario.1.results

scenario.1.2.lgd <- predict(rf.model, first.second.test)
first.second.test$LGD <- scenario.1.2.lgd
```
...
```{r}
first.second.test <- first.second.test %>% mutate(expected_gain = (1 - PD) * 5 * .042 * requested_loan)
...
The third and final scenario, we are faced with a similar situation as the two above, but now each customer has a proposed interest rate. This will affect our expect gain calculation as it will vary from person to person.

...
```{r}
full.model.reduced <- glm(default ~ f8 + f13 + f41 + f51 + f54 + f65 + f66 + f75 + f82 + f143 + f144 + f221 + f222 + f243 + f251 + f259 + f269 + f270 + f290 + f330 + f380 + f381 + f382 + f392 + f397 + f404 + f499 + f563 + f596 + f598 + f599 + f633 + f664 + f774, family = binomial, data = train)
...
```{r}
defaulted <- defaulted %>% dplyr::select(id, f8, f13, f41, f51, f54, f65, f66, f75, f82, f143, f144, f221, f222, f243, f251, f259, f269, f270, f290, f330, f380, f381, f382, f392, f397, f404, f499, f563, f596, f598, f599, f633, f664, f774, loss)
```
...
Expected Loss:
PD * LGD * requested_loan + (1-PD) * customer_default 
...
```{r}
first.second.test <- first.second.test %>% mutate(customer_default = ifelse(PD > .10, 1, 0), expected_gain = (1 - PD) * 5 * .042 * requested_loan, expected_loss = PD * LGD * requested_loan + (1-PD) * customer_default, opportunity = expected_gain - expected_loss)
```
...
scen.2.int.results <- first.second.test %>%
  ...
  summarise(approve = ifelse(opportunity > 13100, 1, 0))
...
Expected Loss:
PD * LGD * requested_loan + (1-PD) * Default 

Expected Gain:
(1 - PD) * 5 * Proposed_Intrest_Rate * requested_loan

```{r}
third.test <- third.test %>% mutate(customer_default = ifelse(PD > .10, 1, 0), expected_gain = (1 - PD) * 5 * Proposed_Intrest_Rate * requested_loan, expected_loss = PD * LGD * requested_loan + (1-PD) * customer_default, opportunity = expected_gain - expected_loss)
```
